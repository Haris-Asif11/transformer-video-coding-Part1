...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        if isinstance(loss_dict, torch.Tensor):
            ...
            '''
            #following code added for testing
            loss_dict1 = {}
            loss_dict1['mse_loss'] = loss_dict['mse_loss']
            loss_dict1['lambda_average_rate'] = loss_dict['lambda_average_rate']
        ...
        if self.iter% 15000 == 0:
            #save_path = f"output_test_31_integrated_model_final/model_weights_num_iter_{self.iter}.pth"
            save_path = os.path.join(self.cfg.OUTPUT_DIR, f"model_weights_num_iter_{self.iter}.pth")

            self.model.save_model_weights(save_path)
            print(f"Saved model weights at num iter {self.iter} to {save_path}")

        if self.iter % 100 == 0:
            #print('MSE Loss:', loss_dict['mse_loss'], '   Rate Loss: ', loss_dict['lambda_average_rate'])
            print('Loss_dict: ', loss_dict)

        if self.iter == self.max_iter-1:
            save_path = os.path.join(self.cfg.OUTPUT_DIR, f"my_integrated_model_weights_final.pth")
            self.model.save_model_weights(save_path)
            #self.model.save_model_weights('output_test_31_integrated_model_final/my_integrated_model_weights_final.pth') #make this path general
...
class AMPTrainer(SimpleTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            model, data_loader, optimizer, cfg,  gather_metric_period, zero_grad_before_forward
        )
